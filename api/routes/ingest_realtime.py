...
def _classify_cluster(
    ...
    # Layer 0: user profile domain rules (highest priority)
    ...
    # Reddit — classify by content
    if "reddit.com" in d:
        content = f"{title.lower()} {description.lower()} {dom_snippet.lower()[:300]}"
        all_focus = FOCUS_SIGNALS + profile.get("extra_focus_signals", [])
        if any(s in content for s in all_focus):
            return "research"
        return "social"

    # YouTube — classify by content
    if d == "youtube.com":
        content = f"{title.lower()} {description.lower()} {dom_snippet.lower()[:300]}"
        all_focus = FOCUS_SIGNALS + profile.get("extra_focus_signals", [])
        if any(s in content for s in all_focus):
